...
1. Run `python core/repository.py search [--after=year]` to search new papers (from DBLP). 
   ...
   After this step, you need to perform manual processing on the newly generated `add.csv` file to **filter out irrelevant papers**, and also **determine research field of each new paper**. Then, all new papers should be manually copied into the `paper.csv` file.
